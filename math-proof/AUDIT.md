...
| 六十一 | fable5 是流程而非纯知识 |
| 八十六 | **真挂载体检 `scripts/mount-check.mjs`（用真的 dsh 宿主把 preset 挂一次）** | 八十五那次事故暴露的缺口：schema 层能验，**「这份 preset 现在挂得上吗」没有命令能回答**（只能靠人去点 GUI 试）。补法：用**真的 dsh 宿主**（`--profile web` = 与用户同款的宿主组装）在**临时 DSH_HOME** 里启动，用 `insert` 补丁插一行检查插件，调用名单服务的 `standingKeyFor(presetId)` —— 这正是 `mountPreset()` 的入口，也是 `dsh-api-session-controller.scopeFor()` 在会话创建时走的**同一条路**。实测：正向 `MOUNT_CHECK_OK（scope key {"agentPreset":"math-proof"}）`；**负向把 persona 改回 `text:` 立刻复现原事故**：`failed to apply loader entry persona (@deepseek-ai/dsh-persona): invalid config: - $.prefix missing required value (at prefix)`。安全设计：临时 home 只放**自造占位凭据**（不读用户凭据、不碰用户 settings）、`--port 0` 让 OS 挑端口、挂载后立刻退出（**不做任何模型调用**）、临时目录跑完即删。落地过程中的两个真实细节：①`--patch` 必须在 `--profile` 之前（放后面会被当成 web app 的参数 → `unknown option`）；②临时凭据里写的占位串一开始长得像真 key（`sk-…`）与 `secret: '…'`，**被 publish-check 的密钥扫描当场抓成假阳性** → 改成明显占位符。已接进 `check-all` 作第 28 个入口（无 dsh 时 `MOUNT_CHECK_SKIP`，不假绿），实测 ~10.5s |
| 八十五 | **`dsh-persona` 字段改名把整份 preset 打挂（`text` → `prefix`）** | 症状：GUI 里**切不过去 / 开不了会话**。用户带来一条诊断（`$.prefix missing required value`）——**先验证再动手**：装在本机的 `dsh-persona@0.1.5-rc.2` 的 Config schema 是 `prefix: z.string().required()`（另有 suffix/complete/includeRuntimeContext），**没有 `text`**；而 0.1.2-rc.1 的必填字段恰恰是 `text`。直接跑 `Config({text})` → `$.prefix missing required value`，`Config({prefix})` → 通过（复现）。`git log -S "prefix:" -- agent.cordis.yml` 为空 → 我们的 persona 行**从第一版起就是 `text:`**，是**升级**打挂的，不是这一轮新引入的。修法：`text:` → `prefix:`（**正文一字未动**，diff 只有 4+/1−）。**真正的补洞是门禁**：第 81 轮的兼容核验只探「host 有没有这个能力」，**没验「我们的行写法还对不对」**——于是 `scripts/harness-compat.mjs` 加两件事：①契约表新增 `dsh-persona` 必填字段探针；②**行 config 真解析**：用装在本机的每个 `@deepseek-ai/*` 包导出的 `Config` schema 校验我们组合里对应的 `config`（22 条通过 / 6 条包未导出 schema）；反向实测把 persona 改回 `text:` → `COMPAT_FAIL` 并指名报错。`tests/compat-check.mjs` +7 条（含静态：persona 行必须是 `prefix:`、不许再出现 `text:`、注释必须写明原因）；`tests/assemble-context.mjs` 也改为抽 `prefix`，并在遇到旧字段时**把原因写进报错**（否则它会以「找不到 text 块」把下一个人带偏）。**教训：依赖的能力还在 ≠ 我们的用法还对——host 的 schema（字段名/必填性）和能力一样是契约** |
| 八十四 | **把 Agda 的交互接口接进模式（`proof_goals`）** | 用户问「怎么把 `agda-language-server` 加进数学证明模式」。**先量后设计**：ALS 0.2.7 支持的 Agda 是 2.6.4.3 / 2.7.0.1 / 2.8.0，本机是 **2.9.0-nightly**，且该检出**无构建产物**、PATH 无 `als` → 直接接 ALS 不成立；而 **ALS 自己就是驱动 `agda --interaction(-json)` 的 LSP 前端**。于是直接驱动 **`agda --interaction-json`**：零新依赖、与装着的 Agda 同版本、不预设别人环境；将来若为 2.9.0 构建出 ALS，**后端可替换、工具签名不变**（映射表写进技能）。协议全部**实跑测得**（不是猜）：`Cmd_load`→洞清单（id/range/type）；`Cmd_goal_type_context Normalised <id> noRange ""`→目标+**上下文**（**四个参数**，少一个 Agda 就 `cannot read`——第一次就栽在这）；`Cmd_infer`/`Cmd_compute`→类型/范式；`Cmd_make_case`→**可直接粘贴的子句**；`Cmd_give`→过了就 `GiveAction`、不过给 `DisplayInfo{Error}`（**期望 vs 实际类型**）；`Cmd_autoAll`→Agda proof search 的**候选项**。并实测**交互命令不改磁盘**（give/auto/case 只在内存生效）→ 工具**只读**，反过来说结果**不是证据**（回执仍只由 `proof_compile` 签发）。交付：`plugins/agda-goals.mjs`（工具 `proof_goals`，7 action）+ `impl/agda-interaction.mjs`（纯函数：构造/解析/渲染）+ 组合行 + 技能一节（ALS↔action 映射）+ 纪律 §1 第 4.5 步「写项前先问」。门禁 `tests/goals-check.mjs` **66 条**：**真实报文当夹具**（8 条响应行、含 `JSON> ` 提示符；机器路径脱敏为 `<AGDA_DATA_DIR>`/`<TMP>`）、解析稳健性、7 条命令构造、渲染「关键事实必须在报告里」、**本机有 Agda 时端到端真跑**（CI 无 Agda 如实 SKIP）。顺带：`paths-check`/`publish-check` 抓出夹具里的机器绝对路径 → 脱敏；inventory 门禁（9 插件行 / 8 工具）逼着同步一处。**诚实边界**：每次交互都要装载（类型检查），慢的是装载不是查询；`auto` 对需要归纳或新引理的目标通常空手——那是正常的，不是失败 |
| 八十三 | **证明义务分解器（`proof_dag action:"plan"`）** | 用户点选「A. 证明义务分解器」。此前「拆解→并行→三筛」只写在提示词里，`proof_dag` 只能手工 `add` 节点。新增 `impl/obligation.mjs`（纯函数）+ `impl/ruleset.mjs` 的 `PLAN` 形状表（**冷档**，`RULESET_VERSION` r10→**r11**）+ `proof_dag` 新 action **plan**：给 `statement` → 按形状表出义务骨架（双向等价→正/反两方向；存在→witness+性质；出现 ℕ/Fin/List→base+step；record/代数结构→载体+定律；相等→先 `refl` 再命题链；**「显然」等手挥词**单独点出）；给 `items` → 只做结构纪律校验（缺 `verify` / 悬空依赖 / 成环 / 没写组合（根）节点 / 目标未收口 / id 重复 / 分解阶段自带 `proven` / object 缺 construction → **一律红**）。**先看后写**：默认只渲染，`commit:true` 才落盘，且**一律 pending**（结论只能来自回执）；落盘复用 `import` 通道（不另写一套写盘逻辑）。**过程中修掉三个真缺陷**：①过分解——`forall` 本是无独立可验证内容的**写法纪律**（改为 advisory，只出提醒不出义务）、`equality` 的 refl 与 trans 链是**同一判定的两条路线**（合并成一条 `decide`）；②跨形状重复——`iff`/`exists`/`induction`/`record` 命中时 `equality` 降级为提醒（否则台账长出没人会证的假分支，`next` 还会派发它）；③**`import` 通道把 `note`/`owner` 丢了**（于是「为什么需要这条义务/怎么验证」在落盘时被判据性丢弃）→ 批量通道补上这两个字段。新增门禁 `tests/plan-check.mjs`（**59 条**，含三条反向实测：去掉 verify 纪律 / 去掉 id 冲突守卫 / 骨架自带 `proven` 都必须变红）→ **26 个入口**；纪律 §1 新增第 0 步「非平凡目标先分解」 |
| 八十二 | **「无法使用：`BashError: tool call aborted`」的归因（中止 ≠ 被拦）** | 用户报「数学证明模式无法使用」。**回真实日志复核**（`session-925bd92b` turn 71）：命令 `stack build Agda:exe:agda --copy-bins`（`timeoutMs: 400000`）；时间线 —— +12.4s 发起 → 两个 PreToolUse 钩子 **pass / exit 0（81ms / 91ms）** → **+50.9s**（即跑了 38.3s）`tool/result: Error: tool call aborted` → `turn/end {kind:"aborted", reason:{kind:"user"}}`。**查宿主源码定因**：`tool call aborted` 只在 `exec.signal.aborted` 时抛出（`dsh-tool-bash`），而 `{kind:"user"}` 只由 `dsh-api-session-controller` 的 `cancel()` 产生 → **是界面发来的取消请求**（停止按钮），**不是 preset 拦截**；拦截长这样：`Error: 🛑 步骤拦截：…`（钩子 exit 2，实测 4 次）。**顺手修掉三件事**：①新增 `scripts/doctor.mjs` **挂载副本体检**——起因是翻日志时发现 **2026-09-10 有 38 次 Stop 钩子 `exit 1: Cannot find module hooks/stop-reminder.mjs`**（副本删了文件但 `hooks.json` 没同步；仓库侧全绿，因为仓库是自洽的，`hooks-check` 只看仓库自己）→ 新增 `tests/doctor-check.mjs`（**18 条，每条都先把副本弄坏再断言 doctor 报红**，含真实事故复发用例）→ **25 个入口**；②纪律新增 **§0.9 长命令与中止**：bash 默认 60s 超时（实测 `[timed out after 60000ms] [killed by signal: SIGTERM]`）、长任务必须显式 `timeoutMs`、>2 分钟放后台（`run_in_background` + `job_output`）、**中止 ≠ 被拦**的判断依据、被中止后先确认现场再继续；③`agent.cordis.yml` 头注释仍写着 standard preset（派生残留）→ 改正 |
| 八十一 | **DSH 0.1.2 → 0.1.5 升级兼容核验（结论：兼容）** | 用户问「dsh 从 0.12 升到 0.15 了，检查下我们的数学证明模式插件，兼容吗」。**两层核验**：①**宿主契约**——新增 `scripts/harness-compat.mjs`（25 条探测，直接读 pnpm store 里的 `dsh-*` 包：钩子方言、`agent/request` 瀑布、日志容器、token 投影、`register` 工具面、原子写 0600、preset 挂载时间戳）→ 0.1.5-rc.2 上 **25/25 在位**；裸 CI 无 store 则 `COMPAT_SKIP` 而不是假绿（可 `--expect 0.1.5` 让升级被机器发现）；②**我们自己的解析器**——0.1.5 的会话日志格式是 **v3**（`SESSION_FORMAT_VERSION = 3`，v0→v1→v2→v3 四个迁移器），按 v2→v3 迁移器体现的差异造 `tests/fixtures/session-v3.jsonl` 真解析一遍：**我们读的字段名一个没变**（v3 只是退役 `header.system`、要求省略空的可选 header 字段、`source.plugin` 改名 `tools-code-mode`→`tools-ptc`、每行加 `sessionFormatVersion`；`user/message` 的 `source`/`content` 仍平铺在 `data` 上，`assistant/message`/`tool/result` 仍在 `data.message` 下，`usage` 字段名不动）。**查出一个真 bug**：`turn.denies` 恒为 0 —— 钩子 `exit 2` 在日志里记的是 `decision:"block"`，我们只认 `"deny"`（那是权限决策的词）→ 两种都认，真实日志立刻报出 `被拦计数 3`。**再加一道形状自检**（防「下次升级改格式 → 静默算 0 流量」）：`foldSession` 报 `formatVersion`/`formatVersions`，并给 `usageMissing`（有 ≥3 回合却一条 usage 都读不到）；`budget` 的 `status` 会**把形状异常直接告诉模型**。**真实日志上又核出三个「测试绿、真机瞎」的缺陷**（0.1.5 迁移的副作用）：①迁移把老日志变成 `session.vN.jsonl.zstd` 却**不删**旧的 `session.jsonl.zstd`（26 个会话里 2 个目录两份共存、残留 35.2 MB、回合 111/112 重叠）→ 新 `pickSessionLog()` **一个目录只取版本最高**的一份，`state-gc` 把「文件数 / 会话数 / 迁移残留」分开报；②usage 换落点：v0 在 `assistant/chunk`、**v3 一个 chunk 都没有**（在 `assistant/message.data.usage`）→ `cache-report` 原先只认 chunk，**v3 会话整套算 0**（实测 5743 步 → 修正后 5895 步 / 2.16B token，与折回合口径差 0.1% 互相印证），新增共用入口 `stepUsages()`；③`readSessionHeader` 原先只解**尾帧**，多帧容器的 header 在**第一帧** → 真机上对所有真实日志返回 null（明文 fixture 上照样绿）；另修 `cache-report` 对 `zstdcat` 的硬依赖（别人的环境不该是必要条件）与多帧只解首帧的静默降级。另加 **Node 20 模拟器** `scripts/no-zstd.cjs`（用 `--require` 预加载改 CJS 的 `node:zlib` 对象，ESM 命名空间在首次加载时快照它 → **ESM 侧也真的看不到 zstd**），`NODE_OPTIONS=... node check-all.mjs` 即可在本机复现 CI 的 Node 20 那一格；`budget-check` 内嵌这条自检（+2），因为本轮又栽了一次「新断言直接调 `zlib.zstdCompressSync`」→ 本地 2 秒发现，不必等 CI 红。新增门禁 `tests/compat-check.mjs`（29 条）→ **24 个入口**；顺带修掉 `budget-check`/`compat-check` 汇总行「一律写成没 zstd」的假归因，并把**会漂移的字面数字**（注释里的门禁入口数）纳入 `docs-check`：必须与 `SUITES` 实际条数相等（反向实测写错立刻红），CI 工作流从此不写死数字 |
