...
**两条已核对的真实向量**（界面 ↔ 我们的折叠，逐字段一致）：

| 会话 | 本轮用量 | 缓存命中 | 未缓存输入 | 缓存读取 | 输出（推理） |
| --- | --- | --- | --- | --- | --- |
| 某长会话 | `25,178,836` | 99.9% | 26,872 | 25,116,032 | 35,932（9,672） |
| 数学证明模式会话 | `11,924,953` | 99.8% | 22,269 | 11,852,288 | 50,396（33,493） |
| 同一会话（大轮） | `14,897,701` | **98%** | 293,757 | 14,541,696 | 62,248（35,326） |
| 同一会话（小轮） | `180,279` | **81%** | 33,577 | 143,360 | 3,342（1,562） |

四条都进了回归（`tests/budget-check.mjs`），任何字段口径漂移都会立刻变红。

**命中率的显示规则**（照着界面 `formatCacheHitPercent` 实现，因为用户是逐字符核对的）：

1. 分母是 **prompt tokens**（`= 未缓存输入 + 缓存读取 = totalTokens - outputTokens`）；
2. **整数算术**取到 0.1 个百分点，**中值向上**（不用浮点 `toFixed`：`.x5` 的方向会不一致）；
3. **整数时去掉尾随 `.0`**：`98.02% → 98`、`81.02% → 81`，而 `99.8 / 99.9` 保留一位
   （这一条是用户拿界面数字核对时发现的——我原先渲染成 `98.0%`，对不上）；
4. **部分命中绝不显示成 100%**：舍到 100 但仍漏 token 时自动加精度（`999999/1000000 → 99.9999`），
   只有真的全命中才写 `100`。

**预设归属的坑（实测）**：会话 header 里的 `agentPreset` 是**创建时**的值，用户中途在 GUI 换预设**不会**改它——
真实切换落成 `agent-preset/selected` 事件。实测某会话 header 写 `standard`，seq 4 切到 `math-proof`：
按 header 分组会**把它归错档**（报表里 70 轮 math-proof 里有 6 轮就是这么来的）。现在折叠按事件取**生效预设**，
并逐回合记录（切换前后分开算），`sessionTotals` 同时给 `presetAtCreation` 与 `preset`。
